...
// The in-process DLL route requires one process-wide rendezvous because mGBA
// calls the C ABI from its Lua thread. It is removed with the legacy DLL after
// socket parity; application and session state themselves are not global.
